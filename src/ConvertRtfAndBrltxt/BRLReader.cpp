...
//==========================================================================
// RTFtoBRLTXTReader
// RTF reader to convert RTF to BRLTXT
//==========================================================================
RTFDestination* RTFtoBRLTXTReader::NextDestination(int id)
{
	switch (id)
	{
		case rtfIVersion:
...
//==========================================================================
// BRLTXTDestination
// Base class for all destinations that convert to BRLTXT
//==========================================================================
bool BRLTXTDestination::HandleToken(RTFReader& reader, Token& token)
{
	bool handled = true;
...
bool BRLTXTDestination::HandleControlToken(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
...
bool BRLTXTDestination::HandleRtfCharAttr(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
...
bool BRLTXTDestination::HandleRtfSpecialChar(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
	...
	switch (token.rtfMinor)
	{
		...
		case rtfLine:
			...
			// since RTF has this setting as persistent and BRL resets it
			// for each line.
			if (reader.State().PA().leftIndent > 0)
			{
				char tmp[10];

				sprintf_s(tmp, " $to%02d ", TwipsToColumns(reader.State().PA().leftIndent));
...
bool BRLTXTDestination::HandleRtfParAttr(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
...
bool BRLTXTDestination::HandleRtfSecAttr(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
...
bool BRLTXTDestination::HandleRtfDocAttr(RTFReader& reader, ControlToken& token)
{
	bool handled = true;
...
//==========================================================================
// BRLTXTHeaderDestination
// Destination for the RTF header group
//==========================================================================
void BRLTXTHeaderDestination::Begin(RTFReader& reader)
{
	reader.Output()->putString(" $hb ");
}

void BRLTXTHeaderDestination::End(RTFReader& reader)
{
	reader.Output()->putString(" $- ");
}

//==========================================================================
// BRLTXTFooterDestination
// Destination for the RTF footer group
//==========================================================================
void BRLTXTFooterDestination::Begin(RTFReader& reader)
{
	reader.Output()->putString(" $fb ");
}

void BRLTXTFooterDestination::End(RTFReader& reader)
{
	reader.Output()->putString(" $- ");
}

//==========================================================================
// BRLTXTFieldInstrDestination
// Destination for RTF field instructions
//==========================================================================
void BRLTXTFieldInstrDestination::Begin(RTFReader& reader)
{
	m_instruction.m_data.erase();
}

void BRLTXTFieldInstrDestination::End(RTFReader& reader)
{
...
CharOutput* BRLTXTFieldInstrDestination::GetOutput(void)
{
	return(&m_instruction);
}

//==========================================================================
// BRLTXTUserPropsDestination
// Destination for the RTF userprops group
//==========================================================================
bool BRLTXTUserPropsDestination::HandleToken(RTFReader& reader, Token& token)
{
	bool handled = true;
	...
	if (!handled)
		handled = BRLTXTDestination::HandleToken(reader, token);
	return(handled);
}

void BRLTXTUserPropsDestination::Begin(RTFReader& reader)
{
	m_currOutput = &m_ignore;
}

void BRLTXTUserPropsDestination::End(RTFReader& reader)
{	
	FlushProperty(reader);
}

void BRLTXTUserPropsDestination::FlushProperty(RTFReader& reader)
{
...
CharOutput* BRLTXTUserPropsDestination::GetOutput(void)
{
	return(m_currOutput);
}
...
void BRLTXTWriter::Write(RTFReaderState& state, LPCSTR value)
{
	if ( m_bFirstTime )
	...
	if (state.PA().leftIndent != m_parAttr.leftIndent)
	{
		...
		sprintf_s(tmp, " $to%02d ", TwipsToColumns(state.PA().leftIndent));
...
/* For now this code is being removed because we are putting in all the brf settings 
   another place.
	...
	{
		char tmp[10];
		sprintf_s(tmp, " $ml%02d ", TwipsToHorizMargin(margin));
		m_output->putString(tmp);
	...
	{
		char tmp[10];
		sprintf_s(tmp, " $mr%02d ", TwipsToHorizMargin(margin));
		m_output->putString(tmp);
	...
	{
		char tmp[10];
		sprintf_s(tmp, " $mt%02d ", TwipsToVertMargin(margin));
		m_output->putString(tmp);
	...
	{
		char tmp[10];
		sprintf_s(tmp, " $mb%02d ", TwipsToVertMargin(margin));
		m_output->putString(tmp);
	...
	{
		char tmp[10];
		//sprintf_s(tmp, " $pl%02d ", TwipsToVertMargin(state.DA().pageHeight));
		sprintf_s(tmp, " $pl%02d ", 25);	// Hard coded for now
		m_output->putString(tmp);
	...
	if (m_docAttr.pageWidth != ColumnsToTwips(34))
	{
		...
		char tmp[10];
		sprintf_s(tmp, " $pw%02d ", TwipsToColumns(state.DA().pageWidth));
		m_output->putString(tmp);
